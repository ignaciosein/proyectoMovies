      ...
      let {movieId,Title} = req.params
        ...
        console.log("Valor data ",data)
                ...
                if (data[0].contador  >0) {
                    ...
                    res.status(200).render("message", { type: "Error: ", message: `Esta pelicula ya existe en la base de datos`, link: `/search/${Title}`, flag: true })
                } 
                else  { 
                    let data2 = await sql.addFavMovie(movieId, email);

              
                    res.status(200).render("message", { type: "Info: ", message: `Se agregó la pelicula a la base de datos`, link: `/search/${Title}`, flag: true })
                }
